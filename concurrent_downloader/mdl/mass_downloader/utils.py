...
def get_mseed_sdsfilename(str_or_fct, network, station, 
                        location, channel, year, julday):
    """
    Helper function getting the filename of a MiniSEED file.

    str_or_fct is a string, and if it contains ``"{network}"``,  ``"{station}"``,
    ``"{location}"``, ``"{channel}"``, ``"{julday}"``, and ``"{endtime}"``
    formatting specifiers, ``str.format()`` is called.
    ...
    """
    julday_3digit = '{:03}'.format(int(julday))
    mseed_path = ("{str_or_fct}/{year}/{network}/{station}/{channel}.D/"
                 "{network}.{station}.{location}.{channel}"
                 ".D.{year}.{julday}")
    path = mseed_path.format(str_or_fct=str_or_fct,
                             year=year, julday=julday_3digit,
                             network=network, station=station,
